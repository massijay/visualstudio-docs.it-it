...
Questa procedura dettagliata viene illustrato come utilizzare JavaScript per creare un semplice matematiche SDK come un Visual Studio Extension \(VSIX\).  La procedura dettagliata è suddivisa nelle parti seguenti:  
...
-   [Per creare un'applicazione di esempio che utilizza il SDK](../extensibility/walkthrough-creating-an-sdk-using-javascript.md#createSampleApp)  
  
 Per JavaScript, non è disponibile alcun tipo di progetto libreria di classi. In questa procedura dettagliata viene creato il file di esempio arithmetic.js direttamente nel progetto VSIX. In pratica, si consiglia innanzitutto di compilare e testare i file JavaScript e CSS come un'applicazione Windows Store, ad esempio, utilizzando il **applicazione vuota** modello, prima di inserirli in un progetto VSIX.  
...
2.  Nell'elenco delle categorie di modelli, in **Visual c\#**, selezionare **estendibilità**, quindi selezionare il **progetto VSIX** modello.  
  
3.  Nel **nome** testo specificare `SimpleMathVSIX` e scegliere il **OK** pulsante.  
  
4.  Se il **guidata pacchetto di Visual Studio** viene visualizzato, scegliere il **Avanti** pulsante il **iniziale** pagina e quindi su **pagina 1 di 7**, scegliere il **Fine** pulsante.  
  
     Sebbene il **Progettazione manifesto** viene aperto, ci limiteremo a questa procedura dettagliata semplice modificando direttamente il file manifesto.  
  
5.  In **Esplora**, aprire il menu di scelta rapida per il file source.extension.vsixmanifest e quindi scegliere **Visualizza codice**. Utilizzare questo codice per sostituire il contenuto esistente nel file.  
...
7.  Nel **dati** categoria, selezionare **file XML**, denominare il file `SDKManifest.xml`, scegliere il **Aggiungi** pulsante.  
...
12. In **Esplora**, scegliere il menu di scelta rapida per il progetto SimpleMathVSIX **Aggiungi**, **nuova cartella**, e quindi denominare la cartella `Redist`.  
...
##  <a name="createSampleApp"></a> Per creare un'applicazione di esempio che utilizza il SDK  
  
1.  Nella barra dei menu scegliere **File**, **Nuovo**, **Progetto**.  
  
2.  Nell'elenco delle categorie di modelli, in **JavaScript**, selezionare **Windows Store**, quindi selezionare il **applicazione vuota** modello.  
...
5.  In **Windows**, scegliere **estensioni**, e si noti che **semplici operazioni matematiche** viene visualizzato.  
  
6.  Selezionare il **semplici operazioni matematiche** casella di controllo e quindi scegliere il **OK** pulsante.  
  
7.  In **Esplora**, in **riferimenti**, si noti che il **semplici operazioni matematiche** riferimento viene visualizzato. Espandere il nodo e notare che è una cartella \\js\\ che includa arithmetic.js. È possibile aprire arithmetic.js per verificare che il codice sorgente è stato installato.  
...
11. Premere il tasto F5 per compilare ed eseguire l'applicazione.  
