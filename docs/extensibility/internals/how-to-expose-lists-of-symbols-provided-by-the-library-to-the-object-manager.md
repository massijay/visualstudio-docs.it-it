---
title: Esporre elenchi dei simboli forniti a Object Manager | Documenti di Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="how-to-expose-lists-of-symbols-provided-by-the-library-to-the-object-manager"></a>Procedura: esporre elenchi dei simboli forniti dalla libreria di gestione degli oggetti
Gli strumenti per l'esplorazione del simbolo, **Visualizzazione classi**, **Visualizzatore oggetti**, **Visualizzatore chiamate** e **risultati ricerca simbolo**, il passaggio delle richieste per i nuovi dati per il [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] di gestione degli oggetti. Gestione degli oggetti consente di trovare le librerie appropriate e richiede nuovi elenchi dei simboli. Le librerie di rispondono fornendo dati richiesti per il [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] gestione oggetto tramite il <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>interfaccia.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2> Il [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] manager oggetto chiama i metodi in <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>interfaccia per ottenere i dati e lo utilizza per compilare o aggiornare le visualizzazioni degli strumenti per l'esplorazione del simbolo.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>  
  
 Una libreria può visualizzare le richieste di dati quando viene richiamato lo strumento, il nodo è espanso o la visualizzazione viene aggiornata. Quando uno strumento per l'esplorazione del simbolo viene richiamato per la prima volta, il gestore di oggetti richiede la libreria per fornire l'elenco di livello superiore. Quando si espande un nodo di elenco, la libreria fornisce un elenco di elementi figlio in tale nodo. Ogni richiesta di gestione di oggetto contiene un indice dell'elemento di interesse. Per visualizzare un nuovo elenco, il gestore di oggetti deve determinare il numero di elementi presenti nell'elenco, il tipo di elementi, i relativi nomi, accessibilità e altre proprietà.  
  
> [!NOTE]
>  Gli esempi di codice gestito seguenti illustrano come fornire gli elenchi di simboli tramite l'implementazione di <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>interfaccia.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2> Il gestore di oggetti chiama i metodi in questa interfaccia e utilizza i dati ottenuti per inserire o aggiornare gli strumenti per l'esplorazione del simbolo.  
>   
>  Per l'implementazione di provider di codice nativo simboli, utilizzare il <xref:Microsoft.VisualStudio.Shell.Interop.IVsObjectList2>interfaccia.</xref:Microsoft.VisualStudio.Shell.Interop.IVsObjectList2>  
  
## <a name="providing-lists-of-symbols-to-the-object-manager"></a>Fornire elenchi di simboli per il gestore oggetti  
  
#### <a name="to-provide-lists-of-symbols-to-the-object-manager"></a>Per fornire elenchi di simboli per il gestore oggetti  
  
1.  Ottenere il numero di elementi nell'elenco di simboli mediante l'implementazione di <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetItemCount%2A>(metodo).</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetItemCount%2A> Nell'esempio seguente viene illustrato il modo in cui il gestore oggetti Ottiene le informazioni sul numero di elementi nell'elenco.  
...
2.  Ottenere informazioni sulle categorie e gli attributi di un elemento di elenco specificato mediante l'implementazione di <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetCategoryField2%2A>(metodo).</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetCategoryField2%2A> Le categorie di elementi vengono specificate nel <xref:Microsoft.VisualStudio.Shell.Interop.LIB_CATEGORY>enumerazione.</xref:Microsoft.VisualStudio.Shell.Interop.LIB_CATEGORY> Nell'esempio seguente viene illustrato il modo in cui il gestore oggetti Ottiene gli attributi degli elementi per una determinata categoria.  
...
3.  Ottenere la rappresentazione testuale di un elemento di elenco specificato mediante l'implementazione di <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetTextWithOwnership%2A>(metodo).</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetTextWithOwnership%2A> Nell'esempio seguente viene illustrato come ottenere un nome completo di un determinato elemento.  
...
5.  Informazioni su se una determinata voce dell'elenco è espandibile implementando il <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetExpandable3%2A>(metodo).</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetExpandable3%2A> Nell'esempio seguente viene illustrato come ottenere le informazioni su se un determinato elemento può essere espanso.  
...
6.  Ottenere un elenco figlio di simboli di un elemento di elenco specificato mediante l'implementazione di <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetList2%2A>(metodo).</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetList2%2A> Nell'esempio seguente viene illustrato come ottenere un elenco figlio di simboli di un determinato elemento per **chiamare** o **chiamanti** grafici.  
