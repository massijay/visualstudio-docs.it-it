---
title: "Ottenere valori locali | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
ms.topic: "article"
helpviewer_keywords: 
  - "valutazione dell'espressione, i valori locali"
  - "debug [debug SDK], i valori locali"
  - "valutazione dell'espressione, il recupero di valori locali"
...
1.  Ottiene nome oggetto locale, proprietà e gli attributi di [FIELD\_INFO](../../extensibility/debugger/reference/field-info.md) struttura compilato quando la classe è stata creata e inizializzata.  
  
2.  Ottiene il tipo del locale dal [IDebugField](../../extensibility/debugger/reference/idebugfield.md) oggetto.  
  
3.  Ottiene il valore del locale dal `IDebugField` oggetto. Questo campo è associato alla posizione di memoria dell'oggetto locale utilizzando il [IDebugBinder](../../extensibility/debugger/reference/idebugbinder.md) oggetto e il valore viene ottenuto da risultante [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) oggetto.  
  
4.  Restituisce proprietà tutte le richieste in un [DEBUG\_PROPERTY\_INFO](../../extensibility/debugger/reference/debug-property-info.md) struttura.  
  
## Codice gestito  
 In questo esempio viene illustrata un'implementazione di `IDebugProperty2::GetPropertyInfo` per un metodo locale nel codice gestito. Viene inoltre una funzione di supporto, `Field.GetType`, che viene utilizzato per ottenere il tipo del campo.`Field.GetValue` viene visualizzato [Valutazione delle variabili locali](../../extensibility/debugger/evaluating-locals.md). La funzione di supporto `Field.MapModifiersToAttributes` \(non mostrato\) converte semplicemente un campo [FIELD\_MODIFIERS](../../extensibility/debugger/reference/field-modifiers.md) flag [DBG\_ATTRIB\_FLAGS](../../extensibility/debugger/reference/dbg-attrib-flags.md) valori.  
...
## Codice non gestito  
 In questo esempio viene illustrata un'implementazione di `IDebugProperty2::GetPropertyInfo` per un metodo locale nel codice non gestito. Viene inoltre due funzioni di supporto, `FieldGetType` e `FieldGetValue` utilizzati per ottenere il campo tipo e valore, rispettivamente. Si noti che `VARIANT`s vengono utilizzati per il valore del campo e di tipo come un `VARIANT` in grado di gestire un'ampia gamma di tipi di valore. In questa implementazione, `FieldGetValue` restituisce un [IDebugField](../../extensibility/debugger/reference/idebugfield.md) oggetto che in seguito convertito in un valore in una chiamata a `FieldGetPrimitiveValue` \(mostrato in [Valutazione delle variabili locali](../../extensibility/debugger/evaluating-locals.md)\).  
