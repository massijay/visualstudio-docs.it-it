---
title: "Esempio di implementazione di modifica dei valori | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
manager: "ghogen"
---
# Esempio di implementazione di modifica dei valori
...
>  In Visual Studio 2015, questa modalità di implementazione di analizzatori di espressioni è deprecata. Per informazioni sull'implementazione analizzatori di espressioni CLR, vedere [gli analizzatori di espressioni CLR](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) e [esempio analizzatore di espressioni gestite](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample).  
 ...
 Ogni locale nella **variabili locali** finestra dispone di un [IDebugProperty2](../../extensibility/debugger/reference/idebugproperty2.md) oggetto associato. Questo `IDebugProperty2` oggetto contiene nome, valore e tipo di oggetto locale. Quando un utente modifica il valore di una variabile locale, Visual Studio chiama [SetValueAsString](../../extensibility/debugger/reference/idebugproperty2-setvalueasstring.md) per aggiornare il valore della variabile locale in memoria. In questo esempio, locale è rappresentata dalla `CFieldProperty` classe che implementa il `IDebugProperty2` interfaccia.  
  
> [!NOTE]
>  Per **espressioni di controllo** e **controllo immediato** espressioni, il valore da modificare è rappresentato dalla `CValueProperty` classe dell'esempio MyCEE. Tuttavia, l'implementazione di `IDebugProperty2::SetValueAsString` è identico a quello illustrato di seguito.  
...
2.  Associa l'oggetto associato [IDebugField](../../extensibility/debugger/reference/idebugfield.md) dell'oggetto nella posizione di memoria e produrre un [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) oggetto.  
  
3.  Converte il valore in una serie di byte.  
  
4.  Chiamate [SetValue](../Topic/IDebugObject::SetValue.md) per archiviare i byte in memoria.  
  
## Codice gestito  
 Questa è un'implementazione di `IDebugProperty2::SetValueAsString` nel codice gestito.  
...
## Codice non gestito  
 Questa è un'implementazione di `IDebugProperty2::SetValueAsString` nel codice gestito. La funzione di supporto `FieldCoerceValueType` \(non mostrati\) impone un `VARIANT` da un tipo specifico e che rende il valore è uno dei tipi `FieldSetValue` in grado di gestire.  
