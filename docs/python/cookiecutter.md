...
Cookiecutter richiede Python 3.3 o versione successiva (a 32 o 64 bit) o Anaconda 3 4.2 o versione successiva (a 32 o 64 bit). Se non è disponibile un interprete Python appropriato, Visual Studio visualizzerà un avviso. Se si installa un interprete Python mentre Visual Studio è in esecuzione, fare clic sul pulsante Home sulla barra degli strumenti di Cookiecutter per rilevare l'interprete appena installato.

Una volta installato, selezionare **Visualizza > Cookicutter Explorer** per aprire la finestra:

![Finestra principale di Cookiecutter](~/docs/python/media/cookiecutter-overview.png)

## <a name="cookiecutter-workflow"></a>Flusso di lavoro di Cookiecutter

Il processo per l'utilizzo di Cookiecutter include la ricerca e selezione di un modello, la clonazione del modello nel computer locale, l'impostazione delle opzioni desiderate e quindi la creazione di codice dal modello, come descritto nelle sezioni seguenti.
...
![Pagina delle opzioni di Cookiecutter](~/docs/python/media/cookiecutter-template-options.png)
...
È possibile personalizzare i valori predefiniti per opzioni specifiche con un file di configurazione dell'utente. Quando l'estensione Cookiecutter rileva un file di configurazione dell'utente, sovrascrive i valori predefiniti del modello con i valori predefiniti della configurazione dell'utente. Questo argomento è trattato nella sezione [User Config](https://cookiecutter.readthedocs.io/en/latest/advanced/user_config.html) (Configurazione dell'utente) della documentazione di Cookiecutter.
...
![Opzioni di Cookiecutter](~/docs/python/media/cookiecutter-tools-options.png)
...
È possibile migliorare questi aspetti specificando metadati aggiuntivi nel file `cookiecutter.json` specifico per Visual Studio (ignorato dall'interfaccia della riga di comando di Cookiecutter). Tutte le proprietà sono facoltative:

| Proprietà | Descrizione |
| --- | --- |
| Label | Specifica ciò che viene visualizzato sopra l'editor per la variabile, anziché il nome della variabile. |
| Description | Specifica che la descrizione comando visualizzata per il controllo di modifica mostrerà questa descrizione, anziché il valore predefinito per tale variabile. |
| URL | Cambia l'etichetta in collegamento ipertestuale, con una descrizione comando che mostra l'URL. Facendo clic sul collegamento ipertestuale, il browser predefinito dell'utente verrà aperto su tale URL. |
...
Per consentire a questi scenari, Visual Studio cerca i metadati estesi in `cookiecutter.json` che descrivono i comandi da eseguire dopo l'apertura dei file generati in Esplora soluzioni o dopo l'aggiunta dei file a un progetto esistente. L'utente può comunque rifiutare esplicitamente di eseguire le attività deselezionando **Run additional tasks on completion** (Esegui attività aggiuntive al completamento) nelle opzioni del modello.
...
Usare una matrice per più argomenti. Per le opzioni, suddividere l'opzione e il relativo valore in argomenti separati, assicurandosi di usare correttamente le virgolette. Ad esempio:
...
Si noti che il comando `Python.InstallProjectRequirements` funziona solo per l'aggiunta di file a un progetto esistente, perché viene elaborato dal progetto Python in Esplora soluzioni e non esiste alcun progetto che possa ricevere il messaggio in Esplora soluzioni - Visualizzazione cartelle. Si tratta di una limitazione che si spera di superare in una versione futura, con un migliore supporto della visualizzazione cartelle in generale.

## <a name="troubleshooting"></a>Risoluzione dei problemi

### <a name="error-loading-template"></a>Errore durante il caricamento del modello

Alcuni modelli potrebbero usare tipi di dati non validi, ad esempio il tipo booleano, nel codice `cookiecutter.json`. È opportuno segnalare questa situazione all'autore del modello. Fare clic sul collegamento **Issues** (Problemi) nel riquadro di informazioni sul modello.

### <a name="hook-script-failed"></a>Script hook non riuscito

Alcuni modelli possono usare script di post-generazione non compatibili con l'interfaccia utente di Cookiecutter. Ad esempio, gli script che richiedono input all'utente avranno esito negativo perché non è disponibile una console terminale.
