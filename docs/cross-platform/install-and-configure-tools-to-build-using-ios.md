...
manager: "ghogen"
---
# Installare e configurare gli strumenti per la compilazione con iOS
...
> [!NOTE]
>  Per informazioni sull'uso dei servizi di Mac ospitati nel cloud anziché su Mac, vedere [Compilare e simulare iOS nel Cloud](../Topic/Build%20and%20Simulate%20iOS%20in%20the%20Cloud.md). Le istruzioni riguardano la compilazione con Visual Studio Tools per Apache Cordova. Per usare le istruzioni per compilare con Visual C\+\+ per lo sviluppo di app per dispositivi mobili multipiattaforma, sostituire l'agente vcremote per vs\-mda\-remote.  
...
-   Dall'app Terminal nel Mac, immettere:  
     ...
     L'agente remoto verrà avviato con una directory di compilazione predefinita ~\/vcremote. Per le opzioni di configurazione aggiuntive, vedere [Configurare l'agente remoto nel Mac](#ConfigureMac).  
...
#### Per configurare l'agente remoto da Visual Studio  
...
-   Per specificare un percorso per i file dell'agente remoto, immettere:  
     ...
     dove *directory\_path* è il percorso nel Mac in cui inserire file di log, build e certificati del server. Per impostazione predefinita, questo percorso è \/Users\/*nome utente*\/vcremote. In questa posizione, le build vengono organizzate per numero.  
...
-   Per eseguire l'agente usando un file di configurazione al posto dei parametri della riga di comando, immettere:  
     ...
     dove *config\_file\_path* è il percorso di un file di configurazione in formato JSON. Le opzioni di avvio e i relativi valori non devono includere trattini.  
  
## Vedere anche  
 [Installare Visual C\+\+ per Sviluppo app per dispositivi mobili multipiattaforma](../cross-platform/install-visual-cpp-for-cross-platform-mobile-development.md)