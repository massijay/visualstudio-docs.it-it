---
title: "Aggiunta di una proprietà di rilevamento alla definizione di un linguaggio specifico di dominio | Documenti di Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
Questa procedura dettagliata viene illustrato come aggiungere una proprietà di rilevamento per un modello di dominio.  
 ...
 Oggetto *rilevamento dominio* è una proprietà che può essere aggiornato dall'utente ma che contiene un valore predefinito che viene calcolato utilizzando i valori di altri elementi o proprietà di dominio.  
  
 Ad esempio, negli strumenti di linguaggio specifico di dominio (strumenti DSL), il nome di proprietà di una classe di dominio ha un valore predefinito che viene calcolato utilizzando il nome della classe di dominio, ma un utente può modificare il valore in fase di progettazione o ripristinare il valore calcolato.  
...
-   Il supporto di strumenti DSL i descrittori di proprietà di rilevamento. Tuttavia, è Impossibile utilizzare la finestra di progettazione DSL per aggiungere una proprietà di rilevamento per una lingua. Pertanto, è necessario aggiungere codice personalizzato per definire e implementare la proprietà di rilevamento.  
  
 Una proprietà di rilevamento ha due stati: rilevamento e aggiornati dall'utente. Le proprietà di rilevamento hanno le caratteristiche seguenti:  
  
-   Quando nello stato di rilevamento viene calcolato il valore della proprietà di rilevamento e il valore viene aggiornato alle altre proprietà modifica il modello.  
  
-   Quando si trova l'aggiornamento da stato utente, il valore della proprietà rilevamento mantiene il valore a cui l'utente ultima impostazione della proprietà.  
  
-   Nel **proprietà** finestra, il **reimpostare** comando per la proprietà di rilevamento è abilitata solo quando la proprietà è contenuta la classe aggiornata per lo stato utente. Il **reimpostare** comando imposta la proprietà di rilevamento dello stato di rilevamento.  
...
-   Nel **proprietà** finestra, quando la proprietà di rilevamento in aggiornato per lo stato utente, il relativo valore viene visualizzato in grassetto.  
...
## <a name="customizing-the-default-dsl-definition"></a>Personalizzazione della definizione DSL predefinito  
 In questa sezione si personalizza la definizione DSL per contenere i seguenti elementi:  
  
-   Namespace proprietà per ogni elemento del modello di rilevamento.  
  
-   Proprietà booleana IsNamespaceTracking per ogni elemento del modello. Questa proprietà indica se la proprietà di rilevamento è nello stato di rilevamento o l'aggiornamento da stato utente.  
  
-   Proprietà Namespace predefinito per il modello. Questa proprietà verrà utilizzata per calcolare il valore predefinito di Namespace proprietà di rilevamento.  
  
-   Proprietà CustomElements calcolato per il modello. Questa proprietà indica la percentuale di elementi che dispongono di uno spazio dei nomi personalizzato.  
...
1.  Nella finestra di progettazione DSL, fare doppio clic su di **ExampleModel** classe di dominio, scegliere **Aggiungi**, quindi fare clic su **DomainProperty**.  
    ...
    2.  Nel **proprietà** finestra per la nuova proprietà, impostare **valore predefinito** a `DefaultNamespace`e impostare **tipo** a **stringa**.  
  
2.  Per il **ExampleModel** dominio classe, aggiungere una proprietà di dominio denominata `CustomElements`.  
  
     Nel **proprietà** finestra per la nuova proprietà, impostare **tipo** a **calcolata**.  
...
4.  Per il **ExampleElement** dominio classe, aggiungere una proprietà di dominio denominata `IsNamespaceTracking`.  
     ...
     Nel **proprietà** finestra per la nuova proprietà, impostare **è esplorabile** a **False**, impostare **valore predefinito** a `true`e impostare **tipo** a **booleano**.  
  
#### <a name="to-update-the-diagram-elements-and-dsl-details"></a>Per aggiornare gli elementi del diagramma e dettagli DSL  
  
1.  Nella finestra di progettazione DSL, fare doppio clic su di **ExampleShape** forma geometrica, scegliere **Aggiungi**, quindi fare clic su **elemento Decorator testo**.  
...
2.  Nella finestra di progettazione DSL, selezionare la linea che connette il **ExampleElement** classe per il **ExampleShape** forma.  
    ...
    1.  Nel **dettagli DSL** finestra, selezionare il **mappe elementi Decorator** scheda.  
  
    2.  Nel **elementi Decorator** elenco, selezionare **NamespaceDecorator**, selezionare la casella di controllo e quindi scegliere il **visualizzare proprietà** elenco, selezionare **Namespace**.  
  
3.  In **Esplora DSL**, espandere il **classi di dominio** cartella, fare doppio clic su di **ExampleElement** nodo e quindi fare clic su **aggiungere di nuovo descrittore di tipo dominio**.  
...
4.  In **Esplora DSL**, selezionare il **il comportamento di serializzazione Xml** nodo.  
    ...
    1.  Nel **proprietà** finestra impostare **personalizzato Post carico** a **True**.  
  
## <a name="transforming-templates"></a>Trasformare i modelli  
 Dopo avere definito le classi di dominio e le proprietà per il linguaggio DSL, è possibile verificare che la definizione DSL può essere trasformata in modo corretto per rigenerare il codice per il progetto.  
...
2.  Il sistema rigenera il codice per la soluzione e Salva Dsldefinition. Per informazioni sul formato XML dei file di definizione, vedere [il file Dsldefinition](../modeling/the-dsldefinition-dsl-file.md).  
  
## <a name="creating-files-for-custom-code"></a>Creazione di file per il codice personalizzato  
 Quando si trasforma tutti i modelli, il sistema genera il codice sorgente che definisce il linguaggio specifico di dominio nei progetti Dsl e DslPackage. In modo da evitare interferenze con il testo generato, scrivere il codice personalizzato nei file che sono diversi dai file di codice generati.  
...
1.  In **Esplora soluzioni**, fare doppio clic sul **DSL** , scegliere **Aggiungi**, quindi fare clic su **nuova cartella**. Denominare la nuova cartella `CustomCode`.  
  
2.  Fare doppio clic su nuovo **codicepersonalizzato appositamente** , scegliere **Aggiungi**, quindi fare clic su **nuovo elemento**.  
  
3.  Selezionare il **File di codice** modello, impostare il **nome** a `NamespaceTrackingProperty.cs`, quindi fare clic su **OK**.  
...
>  Il codice generato per il descrittore di tipo personalizzato per gli strumenti DSL `ExampleModel` chiamate `GetCustomProperties`; tuttavia, gli strumenti DSL genera il codice che implementa il metodo.  
 ...
 La definizione di questo metodo crea il rilevamento descrittore di proprietà per la proprietà di rilevamento di Namespace. Inoltre, gli attributi per la proprietà di rilevamento consentono di **proprietà** finestra per visualizzare correttamente la proprietà.  
...
## <a name="adding-custom-code-for-the-package"></a>Aggiunta di codice personalizzato per il pacchetto  
 Il codice generato definisce un provider della descrizione per la classe di dominio ExampleElement; Tuttavia, è necessario aggiungere codice per indicare il DSL per usare questo provider della descrizione.  
  
#### <a name="to-update-the-dsl-package-to-use-your-custom-type-descriptor"></a>Per aggiornare il pacchetto DSL per l'utilizzo del descrittore di tipo personalizzato  
...
> [!NOTE]
>  Il codice che generano gli strumenti DSL per `ExampleModel` chiamate `GetCustomElementsValue`; tuttavia, gli strumenti DSL genera il codice che implementa il metodo.  
  
 Definizione di `GetCustomElementsValue` metodo fornisce la logica per la proprietà CustomElements calcolato di `ExampleModel`. Questo metodo conta il numero di `ExampleElement` classi di dominio che dispongono di una proprietà che ha un valore utente aggiornato e restituisce una stringa che rappresenta il numero come una proporzione degli elementi totale nel modello di rilevamento di Namespace.  
  
 Inoltre aggiungere un `OnDefaultNamespaceChanged` metodo `ExampleModel`ed eseguire l'override di `OnValueChanged` metodo il `DefaultNamespacePropertyHandler` della classe annidata `ExampleModel` per chiamare `OnDefaultNamespaceChanged`.  
  
 Poiché la proprietà DefaultNamespace viene utilizzata per calcolare il Namespace, proprietà di rilevamento `ExampleModel` deve notificare tutti `ExampleElement` classi di dominio che è stato modificato il valore della proprietà DefaultNamespace.  
...
## <a name="adding-custom-code-for-the-tracking-property"></a>Aggiunta di codice personalizzato per la proprietà di rilevamento  
 ...
 Definizione di questo metodo fornisce la logica per la proprietà CustomElements calcolato di `ExampleModel`. Questo metodo conta il numero di `ExampleElement` classi di dominio che dispongono di un Namespace rilevamento proprietà e l'aggiornamento per lo stato utente e restituisce una stringa che rappresenta il numero come una proporzione degli elementi totale nel modello.  
  
 Inoltre, aggiungere metodi get e set, la proprietà di archiviazione personalizzati Namespace di e archiviazione per la `ExampleElement` classe di dominio.  
  
> [!NOTE]
>  Il codice che generano gli strumenti DSL per `ExampleModel` chiama l'operazione get e set di metodi; tuttavia, gli strumenti DSL genera il codice che implementa i metodi.  
...
> [!NOTE]
>  Il codice che gli strumenti DSL generano chiamate di `OnPostLoadModel` e `OnPostLoadModelAndDiagram` metodi; tuttavia, gli strumenti DSL non generano codice che implementa questi metodi.  
  
#### <a name="to-add-code-to-support-the-custom-post-load-behavior"></a>Per aggiungere codice per supportare il comportamento di post-carico personalizzato  
...
## <a name="testing-the-language"></a>Il linguaggio di test  
 Il passaggio successivo consiste nel compilare ed eseguire la finestra di progettazione DSL in una nuova istanza di [!INCLUDE[vs_current_short](../code-quality/includes/vs_current_short_md.md)] in modo che sia possibile verificare che funzioni correttamente, la proprietà di rilevamento.  
  
#### <a name="to-exercise-the-language"></a>Per verificare la lingua  
  
1.  Nel **compilare** menu, fare clic su **Ricompila soluzione**.  
  
2.  Nel **Debug** menu, fare clic su **Avvia debug**.  
  
     La compilazione sperimentale di [!INCLUDE[vs_current_short](../code-quality/includes/vs_current_short_md.md)] apre il **debug** soluzione che contiene un file di test vuoto.  
...
3.  In **Esplora**, doppio clic sul file Test.trackingPropertyDsl per aprirlo nella finestra di progettazione e quindi fare clic nell'area di progettazione.  
     ...
     Si noti che il **proprietà** finestra per il diagramma, il **Default Namespace** proprietà è **DefaultNamespace**e **elementi personalizzati** proprietà è **0/0**.  
  
4.  Trascinare un **ExampleElement** elemento il **della casella degli strumenti** sulla superficie del diagramma.  
  
5.  Nel **proprietà** finestra per l'elemento, selezionare il **elemento Namespace** , proprietà e modificare il valore da **DefaultNamespace** a **OtherNamespace**.  
...
6.  Nel **proprietà** finestra, fare doppio clic su **elemento Namespace**, quindi fare clic su **reimpostare**.  
     ...
     Fare doppio clic su **elemento Namespace** nuovamente. Il **reimpostare** comando è stato disabilitato perché la proprietà è attualmente in stato di rilevamento.  
  
7.  Trascinare un altro **ExampleElement** dal **della casella degli strumenti** alla superficie del diagramma e impostare il relativo **elemento Namespace** per **OtherNamespace**.  
...
9. Modifica **Default Namespace** per il diagramma da **DefaultNamespace** a **NewNamespace**.  
     ...
     Il **Namespace** delle tracce elemento prima di **Default Namespace** proprietà, mentre il **Namespace** del secondo elemento mantiene il valore l'utente ha aggiornato **OtherNamespace**.  
  
10. Salvare la soluzione e quindi chiudere build sperimentale.  
