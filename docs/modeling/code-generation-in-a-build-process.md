---
title: Generazione del codice in un processo di compilazione | Documenti di Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
[Trasformazione del testo](../modeling/code-generation-and-t4-text-templates.md) possono essere richiamate come parte di [processo di compilazione](http://msdn.microsoft.com/Library/a971b0f9-7c28-479d-a37b-8fd7e27ef692) di un [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] soluzione. Esistono attività di compilazione che sono specializzate nella trasformazione del testo. Le attività di compilazione di T4 eseguono modelli di testo della fase di progettazione e compilano anche modelli di testo (pre-elaborati) della fase di esecuzione.  
 ...
 Ciò significa che non è possibile accedere a elementi quali i nomi dei file di progetto allo stesso modo di quando si compila un modello di testo in MSBuild. È tuttavia possibile [passare informazioni sull'ambiente in modelli di testo e processori di direttive utilizzando parametri di compilazione](#parameters).  
  
##  <a name="a-namebuildservera-configure-your-machines"></a><a name="buildserver"></a>Configurare i computer  
...
[!INCLUDE[modeling_sdk_info](includes/modeling_sdk_info.md)]
 ...
 Se [il server di compilazione](http://msdn.microsoft.com/Library/788443c3-0547-452e-959c-4805573813a9) viene eseguito su un computer in cui [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] non è installato, copiare i file seguenti nel computer di compilazione dal computer di sviluppo. Sostituire ‘*’ con il numero dell'ultima versione.  
...
##  <a name="a-nameparametersa-pass-build-context-data-into-the-templates"></a><a name="parameters"></a>Passare i dati di contesto di compilazione nei modelli  
...
```  
 ...
 In un processore di direttiva, è possibile chiamare <xref:Microsoft.VisualStudio.TextTemplating.ITextTemplatingEngineHost.ResolveParameterValue%2A>:</xref:Microsoft.VisualStudio.TextTemplating.ITextTemplatingEngineHost.ResolveParameterValue%2A>  
...
##  <a name="a-namemsbuilda-using-project-properties-in-assembly-and-include-directives"></a><a name="msbuild"></a>Usare le proprietà del progetto in un assembly e includere le direttive  
 ...
 **Motivo per cui si desidera trasformare i modelli nel server di compilazione? Modelli in Visual Studio sono già stati trasformati prima di archiviare il codice.**  
...
-   Il [utilità TextTransform](../modeling/generating-files-with-the-texttransform-utility.md) può essere utilizzata negli script di comando. Nella maggior parte dei casi è più facile utilizzare MSBuild.  
...
-   [Modelli di testo fase di esecuzione](../modeling/run-time-text-generation-with-t4-text-templates.md) vengono trasformate in fase di esecuzione dell'applicazione.  
...
## <a name="read-more"></a>Altre informazioni  
 ...
 [Oleg Sych: Informazioni su T4: integrazione di](http://www.olegsych.com/2010/04/understanding-t4-msbuild-integration/)
