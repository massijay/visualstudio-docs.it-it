...
L'evento 'event' può trovarsi soltanto sul lato sinistro di \+\= o di \-\= \(tranne quando è utilizzato dall'interno del tipo 'type'\)  
 ...
 All'esterno della classe in cui è definito, un [evento](/dotnet/csharp/language-reference/keywords/event) può solo aggiungere o sottrarre riferimenti. Per altre informazioni, vedere [Eventi](/dotnet/csharp/programming-guide/events/index).  
