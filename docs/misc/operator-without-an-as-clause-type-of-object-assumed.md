...
Una routine di operatore non specifica una clausola `As`.  
 ...
 Una clausola `As` identifica un tipo di dati da associare a un elemento di programmazione. In un'[Operator Statement](/dotnet/visual-basic/language-reference/statements/operator-statement) specifica il tipo di dati del valore che la routine di operatore restituisce al codice chiamante. Se non si include una clausola `As` nell'istruzione `Operator`, il tipo di dati restituito sarà `Object`.  
