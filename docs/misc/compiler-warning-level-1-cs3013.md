...
I moduli aggiunti devono essere contrassegnati con l'attributo CLSCompliant per corrispondere all'assembly  
 ...
 Un modulo compilato con l'opzione del compilatore [\/target:module](../Topic/-target:module%20\(C%23%20Compiler%20Options\).md) è stato aggiunto a una compilazione con [\/addmodule](/dotnet/csharp/language-reference/compiler-options/addmodule-compiler-option). Tuttavia, la conformità del modulo alle specifiche CLS \(Common Language Specification\) non concorda con lo stato CLS della compilazione corrente.  
