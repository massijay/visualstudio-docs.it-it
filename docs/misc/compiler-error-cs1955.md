...
## Esempio  
 Il codice seguente genera l'errore CS1955 poiché il codice sta tentando di richiamare un campo e una proprietà usando un operatore di chiamata al metodo [\(\)](/dotnet/csharp/language-reference/operators/invocation-operator). Non è possibile chiamare un campo o una proprietà, ma è possibile accedere al valore in esso archiviato usando l'operatore di accesso ai membri \( [.](/dotnet/csharp/language-reference/operators/member-access-operator) \).  
