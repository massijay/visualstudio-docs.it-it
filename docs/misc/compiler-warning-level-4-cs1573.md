...
Il parametro 'parameter', diversamente da altri parametri, non contiene tag param corrispondenti nel commento XML per 'parameter'  
 ...
 Quando si usa l'opzione del compilatore [\/doc](/dotnet/csharp/language-reference/compiler-options/doc-compiler-option), un commento è stato specificato per alcuni ma non per tutti i parametri in un metodo. È possibile che si sia dimenticato di inserire un commento per questi parametri.  
