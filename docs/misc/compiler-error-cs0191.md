...
Non è possibile assegnare un valore alla proprietà o all'indicizzatore 'name' perché è di sola lettura  
 ...
 Un campo [readonly](/dotnet/csharp/language-reference/keywords/readonly) può accettare un'assegnazione solo in un costruttore o in una dichiarazione. Per altre informazioni, vedere [Costruttori](/dotnet/csharp/programming-guide/classes-and-structs/constructors).  
  
 L'errore CS0191 viene generato anche se il campo `readonly` è [static](/dotnet/csharp/language-reference/keywords/static) e il costruttore non è contrassegnato come `static`.  
