...
Una dichiarazione di variabile non specifica una clausola `As`.  
 ...
 Una clausola `As` identifica un tipo di dati da associare a un elemento di programmazione. In un'[Dim Statement](/dotnet/visual-basic/language-reference/statements/dim-statement), specifica il tipo di dati di una o più variabili. Se non si include una clausola `As` nell'istruzione `Dim`, il tipo di dati della variabile sarà `Object`.  
