...
Un'istruzione di assegnazione assegna l'indirizzo di una routine di overload a una variabile delegato, ma il compilatore non riesce a risolvere le diverse versioni di overload.  
 ...
 Quando il codice usa l'indirizzo di una routine definito in numerose versioni di overload, il compilatore deve decidere quale overload usare. Cerca una singola versione con un elenco di parametri che corrisponda all'elenco di parametri del delegato. Per altre informazioni, vedere [Overload Resolution](/dotnet/visual-basic/programming-guide/language-features/procedures/overload-resolution).  
