...
L'operatore '\<simbolooperatore\>' non restituisce un valore in tutti i percorsi del codice. In fase di esecuzione, quando viene usato il risultato, potrebbe verificarsi un'eccezione dovuta a un riferimento Null.  
 ...
 L'unico modo per ottenere un valore da una routine di operatore consiste nell'includerla in un'[Return Statement](/dotnet/visual-basic/language-reference/statements/return-statement).  
  
 Se il controllo passa all'istruzione `End Operator`, la routine di operatore restituisce il valore predefinito del tipo di dati della proprietà. Per altre informazioni, vedere la sezione relativa al comportamento in [Function Statement](/dotnet/visual-basic/language-reference/statements/function-statement).  
