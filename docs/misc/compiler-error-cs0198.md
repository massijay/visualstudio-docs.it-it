...
Non è possibile effettuare un'assegnazione a campi del campo statico di sola lettura 'name' \(tranne che in un costruttore statico o in un inizializzatore di variabile\)  
 ...
 Una variabile [readonly](/dotnet/csharp/language-reference/keywords/readonly) deve avere lo stesso utilizzo [static](/dotnet/csharp/language-reference/keywords/static) del costruttore in cui si vuole inizializzarla. Per altre informazioni, vedere [Costruttori statici](/dotnet/csharp/programming-guide/classes-and-structs/static-constructors).  
