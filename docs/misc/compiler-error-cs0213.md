...
Impossibile utilizzare l'istruzione fixed per accettare l'indirizzo di un'espressione già di tipo fixed  
 ...
 Una variabile locale in un metodo [unsafe](/dotnet/csharp/language-reference/keywords/unsafe) o in un parametro è già di tipo fixed \(nello stack\), pertanto non è possibile accettare l'indirizzo di una di queste due variabili in un'espressione di tipo [fixed](/dotnet/csharp/language-reference/keywords/fixed-statement). Per altre informazioni, vedere [Codice unsafe e puntatori](/dotnet/csharp/programming-guide/unsafe-code-pointers/index).  
