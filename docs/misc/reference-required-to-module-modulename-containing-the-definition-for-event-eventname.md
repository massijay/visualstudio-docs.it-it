...
È necessario il riferimento al modulo '\<`modulename`\>' contenente il tipo '\<`eventname`\>'. Aggiungerne uno al progetto.  
 ...
 L'evento è definito in un modulo a cui non si fa riferimento direttamente nel progetto. Il compilatore [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] richiede un riferimento per evitare ambiguità nel caso in cui l'evento venga definito in più moduli.  
