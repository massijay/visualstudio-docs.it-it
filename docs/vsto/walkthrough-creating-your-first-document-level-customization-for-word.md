---
title: "Procedura dettagliata: creazione di una personalizzazione a livello di documento per Word | Microsoft Docs"
ms.custom: ""
ms.date: "02/02/2017"
...
8.  Selezionare **Crea un nuovo documento** e fare clic su **OK**.  
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] crea il progetto **FirstDocumentCustomization** e quindi vi aggiunge il documento **FirstDocumentCustomization** e il file di codice ThisDocument.  Il documento **FirstDocumentCustomization** viene aperto automaticamente nella finestra di progettazione.  
...
2.  Sostituire il gestore eventi `ThisDocument_Startup` con il codice seguente.  Quando il documento viene aperto, questo codice aggiunge un secondo paragrafo di testo al documento.  
    ...
    > [!NOTE]  
    >  Questo codice usa il valore di indice 1 per accedere al primo paragrafo contenuto nella proprietà <xref:Microsoft.Office.Tools.Word.Document.Paragraphs%2A>.  Anche se Visual Basic e Visual C\# usano matrici in base 0, il limite inferiore di matrice della maggior parte delle raccolte del modello a oggetti di Word è 1.  Per altre informazioni, vedere [Scrittura di codice nelle soluzioni Office](../vsto/writing-code-in-office-solutions.md).  
...
-   Personalizzazione dell'interfaccia utente di Word, ad esempio con l'aggiunta di una scheda personalizzata alla barra multifunzione o la creazione di un riquadro azioni personalizzato: [Personalizzazione dell'interfaccia utente di Office](../vsto/office-ui-customization.md).  
  
-   Uso di oggetti estesi di Word fornito da soluzioni Office in Visual Studio per eseguire attività che risultano impossibili con il modello a oggetti di Word \(ad esempio, l'hosting di controlli gestiti nei documenti e nei controlli di Word ai dati con il modello di associazione dati di Windows Form\): [Automazione di Word usando oggetti estesi](../vsto/automating-word-by-using-extended-objects.md).  
...
-   Distribuzione di personalizzazioni a livello di documento per Word: [Distribuzione di una soluzione Office](../vsto/deploying-an-office-solution.md).  
